# Ubuntu, Apache, regfish DNS und certbro

English version: [../../en/examples/ubuntu-apache-certbro.md](../../en/examples/ubuntu-apache-certbro.md)

Dieses Beispiel zeigt einen kompletten Linux-Workflow für `example.certbro.com` auf Ubuntu mit Apache:
...
7. Apache auf HTTPS umstellen
8. Automatische Renewals aktivieren
9. Den Renewal-Flow prüfen
...
sudo apt-get install -y apache2 curl ca-certificates dnsutils ufw
...
printf '<!doctype html><html><body><h1>%s</h1><p>Apache is ready.</p></body></html>\n' "${HOST_FQDN}" | sudo tee "${WEBROOT}/index.html" >/dev/null
```

Einen reinen HTTP-Apache-vHost anlegen:

```sh
sudo tee "/etc/apache2/sites-available/${HOST_FQDN}.conf" >/dev/null <<EOF
<VirtualHost *:80>
...
sudo a2ensite "${HOST_FQDN}.conf"
sudo apache2ctl -t
sudo systemctl reload apache2
```

Diese erste Apache-Validierung und dieser Reload bleiben manuell, weil die Apache-Konfiguration direkt geändert wird, bevor `certbro` beteiligt ist.
...
Nach `certbro issue` selbst ist kein separates `sudo apache2ctl -t` oder `sudo systemctl reload apache2` mehr nötig. Dasselbe gilt für spätere Renewals und Reissues, die von `certbro` ausgeführt werden.

## 8. Apache auf HTTPS umstellen

Die benötigten Module aktivieren und den reinen HTTP-vHost durch Redirect plus TLS-vHost ersetzen:
...
<VirtualHost *:443>
    ...
    SSLEngine on
    SSLCertificateFile ${CERTBRO_DIR}/live/fullchain.pem
    SSLCertificateKeyFile ${CERTBRO_DIR}/live/privkey.pem
...
> Diese einmalige Validierung und dieser Reload bleiben manuell, weil der Apache-vHost nach der Zertifikatsausstellung noch einmal von Hand geändert wird. `certbro` hat derzeit keinen eigenständigen Befehl, der nur Apache testet und reloadet, ohne zugleich einen Deploy-Schritt auszuführen.
...
- `apache2`, das `example.certbro.com` ausliefert
