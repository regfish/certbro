# Ubuntu, Apache, regfish DNS, and certbro

German version: [../../de/examples/ubuntu-apache-certbro.md](../../de/examples/ubuntu-apache-certbro.md)

This example shows a complete Linux setup for `example.certbro.com` on Ubuntu with Apache:
...
7. Switch Apache to HTTPS
8. Enable automatic renewals
9. Verify the renewal flow
...
Create an HTTP-only Apache vhost:
...
This initial Apache validation and reload are still manual because you are changing the Apache configuration directly before `certbro` is involved.
...
No separate `sudo apache2ctl -t` or `sudo systemctl reload apache2` is needed after `certbro issue` itself. The same applies to later renewals and reissues handled by `certbro`.

## 8. Switch Apache to HTTPS

Enable the required modules and replace the HTTP-only vhost with an HTTP-to-HTTPS redirect plus a TLS vhost:
...
> This one-time validation and reload stay manual because you are editing the Apache vhost after the certificate has already been issued. `certbro` does not currently provide a standalone command that only tests and reloads Apache without also performing a deploy step.
...
- `apache2` serving `example.certbro.com`
